...
async function loadTimeseries(audioUrl) {
    ...
    if (tsController) tsController.abort();
    tsController = new AbortController();
    const signal = tsController.signal;

    waveformContainer.innerHTML = `<div class="text-center py-3">Loading...</div>`;

    try {
        const response = await fetch(
            `/mixer/timeseries/?audio_url=${encodeURIComponent(audioUrl)}&speed=${speedControl.value}&pitch=${pitchControl.value}&amplitude=${amplitudeControl.value}`,
            { signal }
        );
        ...
        // else: aborted request, do nothing
    }
}
...
async function loadSpectrogram(audioUrl) {
    ...
    const signal = specController.signal;

    spectrogramContainer.innerHTML = `<div class="text-center py-3">Loading...</div>`;

    try {
        const response = await fetch(
            `/mixer/spectrogram/?audio_url=${encodeURIComponent(audioUrl)}&speed=${speedControl.value}&pitch=${pitchControl.value}&amplitude=${amplitudeControl.value}`,
            { signal }
        );
        const data = await response.json();
        if (data.success) {
        ...
        // else: request was aborted, do nothing
    }
}
...
[speedControl, pitchControl, amplitudeControl].forEach(el => {
    el.addEventListener('change', () => {
        ...
        // Wait 200ms after last change before updating
